  ...
  const handleLogout = async () => {
    try {
      if (apiUser && isAuthenticated) {
        // Use API logout for authenticated users - determine if host based on route or user data
        const isHost =
          window.location.pathname.toLowerCase().includes("/host") ||
          apiUser?.role === "host";
        await apiLogout(isHost);
