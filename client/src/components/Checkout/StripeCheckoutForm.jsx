...
const StripeCheckoutForm = ({ bookingData, onPaymentSuccess }) => {
  ...
  useEffect(() => {
    // Skip payment intent creation in development mode
    const isDevelopment =
      import.meta.env.MODE === "development" || import.meta.env.DEV;

    if (isDevelopment) {
      console.log("🛠️ Development mode: Skipping payment intent creation");
      setClientSecret("dev_mode_no_secret_needed");
      return;
    }
  ...
  const handleSubmit = async (event) => {
    ...
    // Development mode bypass - skip actual payment processing
    const isDevelopment =
      import.meta.env.MODE === "development" || import.meta.env.DEV;
      ...
      {(import.meta.env.MODE === "development" || import.meta.env.DEV) && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-300 rounded-lg">
          <p className="text-sm text-yellow-800 font-medium">
            🛠️ Development Mode: Payment will be simulated (No real payment
            processing)
          </p>
        </div>
      )}
        ...
        <>
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-4">
              ...
              {import.meta.env.MODE === "development" || import.meta.env.DEV ? (
                <div className="p-4 border-2 border-primary-200 rounded-xl bg-white">
                  <p className="text-sm text-gray-600">
                    💳 In development mode, no card details are required
                  </p>
                </div>
              ) : (
            ...
            >
              {processing ? (
                ...
                <>
                  <FaLock />
                  {import.meta.env.MODE === "development" || import.meta.env.DEV
                    ? `Confirm & Pay (Dev Mode) - ₦${bookingData.totalAmount?.toLocaleString()}`
